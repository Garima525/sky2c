...
class SevenCorners{
	...
	**/
	public static function processRequestVisitors($company_id,$maximum_sevencorner,$deductible_sevencorner,$fdate_dd,$fdate_rd,$seven_living_country_conty_code,$seven_destination_country_conty_code,$seven_applicant_age,$seven_spouse_age,$seven_dependent_childern_0_9,$seven_dependent_childern_10_17,$seven_stataus_spouse,$seven_stataus_child09,$seven_stataus_child1017,$applicant_age_integer){		
		...
		if($company_id==304){
			$policyId = 31297; $productGroupId = 1116;
			if($maximum_sevencorner==15000){ $policy_maximum = "$15,000"; }else if($maximum_sevencorner==25000){ $policy_maximum = "$25,000"; }else if($maximum_sevencorner==50000){ 	$policy_maximum = "$50,000"; }else if($maximum_sevencorner==100000){ $policy_maximum = "$100,000"; }else if($maximum_sevencorner==500000){ $policy_maximum = "$500,000";	
			}else if($maximum_sevencorner==1000000){ $policy_maximum = "$1,000,000"; }		
			
			/*To Set Policy Deductable - As per seven corners values*/
			if($deductible_sevencorner==0){ $policy_deductible = "$0"; }else if($deductible_sevencorner==100){ $policy_deductible = "$100"; }else if($deductible_sevencorner==250){ $policy_deductible = "$250"; }else if($deductible_sevencorner==500){ $policy_deductible = "$500"; }else if($deductible_sevencorner==1000){ $policy_deductible = "$1,000";
			}else if($deductible_sevencorner==2500){ $policy_deductible = "$2,500"; }			
			$policy_max_keyword = "MMPgm_BND"; $policy_deduct_keyword = "DedPgm_BND"; $InOut_BND = "Travel Including the United States";
		}
		...
		$poststring = array("saveQuote"=>'',"quoteIdentifier"=>"1088EDF3-875A-4C32-BE3D-2F428E81DC49","quoteRequestCount"=>1,"policyQuoteRequests"=>array(array("policyId"=>$policyId,"productGroupId"=>$productGroupId,"effectiveDate"=>$fdate_dd,"expirationDate"=>$fdate_rd,"fields" =>array(array("code" =>"USArrivalDate_FLD","value"=>$fdate_dd),array("code"=>"HomeCountryResidence_FLD","value"=>$seven_living_country_conty_code),array("code"=>"DestinationCountrySingleSelect_FLD","value"=>$seven_destination_country_conty_code),array("code"=>$policy_max_keyword,"value"=>$policy_maximum),array("code"=>$policy_deduct_keyword,"value"=>$policy_deductible),array("code"=>"HSC_BND","value"=>"Declined"),array("code"=>"AccOpt_BND","value"=>"Declined"),array("code"=>"TrpCan_BND","value"=>"Declined"),array("code"=>"OIC_BND","value"=>"Declined"),array("code"=>"OCREV_BND","value"=>"Declined"),array("code"=>"OBE_BND","value"=>"Declined"),array("code"=>"OESA_BND","value"=>"Declined"),array("code"=>"InOut_BND","value"=>$InOut_BND)),"personFields" => $rm_empty_personIdentifiers)),"persons" => $rm_empty_personIdentifiers_data,"primaryMemberAddresses" => array(array("type" => "Billing","addressLine1" => "","city" => "","countryCode" => $seven_living_country_conty_code,"state" => "","postalCode" => "")));
...
$array_companies = array(304);
foreach($array_companies as $row){
	$time_start = microtime(true);
	$company_id = $row;	
	
	if($company_id=="304"){
		$maximum_sevencorner = 50000; $deductible_sevencorner = 2500; 
	}
	
	if($company_id==304){
		$company_name = "MCIS Flexible Travel Insurance - Including USA";
	}
	...
	echo "This page Not calling the API URL, Just Generating the Quote JSON Strings and printed, PHP Scripts Alone Running Time!!!";
