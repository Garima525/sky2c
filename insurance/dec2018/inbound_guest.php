...
class SevenCorners{
	...
	**/
	public static function processRequestVisitors($company_id,$maximum_sevencorner,$deductible_sevencorner,$fdate_dd,$fdate_rd,$seven_living_country_conty_code,$seven_destination_country_conty_code,$seven_applicant_age,$seven_spouse_age,$seven_dependent_childern_0_9,$seven_dependent_childern_10_17,$seven_stataus_spouse,$seven_stataus_child09,$seven_stataus_child1017,$applicant_age_integer){		
		...
		/*Making the SevenCorners Quote Strings*/
		//Inbound Guest
		if($company_id==9){
			$policyId = 29955;
			$productGroupId = 712;
			...
			if($maximum_sevencorner==25000){
				$policy_maximum = "Plan A";
			}else if($maximum_sevencorner==45000){
				$policy_maximum = "Plan B";
			}else if($maximum_sevencorner==65000){
				$policy_maximum = "Plan C";
			}else if($maximum_sevencorner==85000){
				$policy_maximum = "Plan D";	
			}else if($maximum_sevencorner==120000){
				$policy_maximum = "Plan E";
			}
				
			if($applicant_age_integer>=70){
				if($maximum_sevencorner==40000){
					$policy_maximum = "Plan J";
				}else if($maximum_sevencorner==60000){
					$policy_maximum = "Plan K";
				}else if($maximum_sevencorner==100000){
					$policy_maximum = "Plan L";
				}
			}
			
			/*To Set Policy Deductable - As per seven corners values*/
			if($deductible_sevencorner==0){
				$policy_deductible = "$0";
			}else if($deductible_sevencorner==50){
				$policy_deductible = "$50";
			}else if($deductible_sevencorner==100){
				$policy_deductible = "$100";
			}else if($deductible_sevencorner==200){
				$policy_deductible = "$200";
			}
				
			$policy_max_keyword = "PlnTyp_BND";
			$policy_deduct_keyword = "DedOcc_BND";
			$InOut_BND = "No";
			$HSC_BND = "No";
		}
		...
		$rm_empty_personIdentifiers_data_final_set = $rm_empty_personIdentifiers_data[0];
			...
			$poststring = array(
				"saveQuote"=>'',
				"quoteIdentifier"=>"1088EDF3-875A-4C32-BE3D-2F428E81DC48",
				"quoteRequestCount"=>1,
				"policyQuoteRequests"=>array(
				array(
				"policyId"=>$policyId,
				"productGroupId"=>$productGroupId,
				"effectiveDate"=>$fdate_dd,
				"expirationDate"=>$fdate_rd,				
				"fields" =>array(array("code" =>"USArrivalDate_FLD","value"=>$fdate_dd),
				array("code"=>"PassportIssuerCountry_FLD","value"=>$seven_living_country_conty_code),
				array("code"=>"DestinationCountry_FLD","value"=>$seven_destination_country_conty_code),				
				array("code"=>$policy_max_keyword,"value"=>$policy_maximum),
				array("code"=>$policy_deduct_keyword,"value"=>$policy_deductible),				
				array("code"=>"170_DYN","value"=>"True"),
				array("code"=>"HSC_BND","value"=>$HSC_BND),
				array("code"=>"InOut_BND","value"=>$InOut_BND)),					
				"personFields" => $rm_empty_personIdentifiers
				)),
				"persons" => $rm_empty_personIdentifiers_data,
				"primaryMemberAddresses" => array(array(
				"type" => "Billing",
				"addressLine1" => "",
				"city" => "",
				"countryCode" => $seven_living_country_conty_code,
				"state" => "",
				"postalCode" => ""
				)));
...
$array_companies = array(9);
foreach($array_companies as $row){
	$time_start = microtime(true);
	$company_id = $row;		
	if($company_id=="9"){	
			$maximum_sevencorner = 25000;
			$deductible_sevencorner = 100;
			if($applicant_age_integer>=70){ $maximum_sevencorner=40000; $deductible_sevencorner = 200;}	
	}
	
	if($company_id==9){
		$company_name = "Inbound Guest";
	}
	...
	echo "Policy Maximum: $25,000<br>";
	echo "Deductible:  $100<br>";
	echo "*************************************************<br><br>";
