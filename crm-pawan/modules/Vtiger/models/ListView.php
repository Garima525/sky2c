...
class Vtiger_ListView_Model extends Vtiger_Base_Model {
	...
	public function getListViewEntries($pagingModel) {
		...
		} else if(empty($orderBy) && empty($sortOrder) && $moduleName != "Users" && $moduleName != 'Leads'){
			//List view will be displayed on recently created/modified records
			$listQuery .= ' ORDER BY vtiger_crmentity.modifiedtime DESC';
		}
		
		if($orderbbyy==0 && $moduleName == 'Leads'){
			$listQuery .= ' ORDER BY vtiger_leaddetails.leadid DESC';
		}	
