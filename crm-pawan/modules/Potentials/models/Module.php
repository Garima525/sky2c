...
class Potentials_Module_Model extends Vtiger_Module_Model {
	 ...
	 * @return <Array>
	 */
	public function getPotentialsCountBySalesPerson($dateFilter) {
		$db = PearDatabase::getInstance();
		...
		if(!empty($dateFilter)){
			$dateFilterSql = ' AND createdtime BETWEEN "'.$dateFilter['start'].'" AND "'.$dateFilter['end'].'"';
		}
	 ...
	 * @return <Array>
	 */
	function getPotentialsPipelinedAmountPerSalesPerson($dateFilter) {
		$db = PearDatabase::getInstance();
		//TODO need to handle security
		$params = array();
		$dateFilterSql = '';
	 ...
	 * @return <Array>
	 */
	function getPotentialTotalAmountBySalesStage($dateFilter){
		//$currentUser = Users_Record_Model::getCurrentUserModel();
		$db = PearDatabase::getInstance();
		$dateFilterSql = "";
		if(!empty($dateFilter)){
		...
		foreach ($picklistValues as $key => $picklistValue) {
			$result = $db->pquery('SELECT SUM(amount) AS amount FROM vtiger_potential
								   INNER JOIN vtiger_crmentity ON vtiger_potential.potentialid = vtiger_crmentity.crmid
								   AND deleted = 0 '.Users_Privileges_Model::getNonAdminAccessControlQuery($this->getName()).' WHERE sales_stage = ?   '.$dateFilterSql.'', array($picklistValue));
