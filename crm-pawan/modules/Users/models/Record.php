...
class Users_Record_Model extends Vtiger_Record_Model {
	...
	/* Created by : Jeyaram.A: */
	public static function isUserViewed($id){
		$db = PearDatabase::getInstance();
		$mode = "0";
		$sql = "UPDATE vtiger_leadscf SET cf_1254=? WHERE leadid=?";
		$db->pquery($sql, array($mode,$id));
	}
	
	public static function isUserViewedActive($id){
		$db = PearDatabase::getInstance();
		$mode = "1";
		$sql = "UPDATE vtiger_leadscf SET cf_1254=? WHERE leadid=?";
		$db->pquery($sql, array($mode,$id));
