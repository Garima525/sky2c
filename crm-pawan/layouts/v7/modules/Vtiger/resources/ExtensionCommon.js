...
Vtiger.Class("Vtiger_ExtensionCommon_Js",{}, {
    ...
    pageJump : function(container) {
		...
		var element2 = container.find('#totalPageCount2');
		var totalPageNumber2 = element2.text();
		var pageCount2;
		
		if(totalPageNumber2 === ""){
			var totalCountElem = container.find('#totalCount2');
			var totalRecordCount = totalCountElem.val();
			if(totalRecordCount !== '') {
				var recordPerPage = container.find('#pageLimit2').val();
				if(recordPerPage === '0') recordPerPage = 1;
				pageCount2 = Math.ceil(totalRecordCount/recordPerPage);
				if(pageCount2 === 0){
					pageCount2 = 1;
				}
				element.text(pageCount2);
				return;
			}
		}
    ...
    totalNumOfRecords : function (currentEle, container) {
		...
		if(totalNumberOfRecords === '') {
			thisInstance.getPageCount().then(function(data){
				...
				container.find('ul#listViewPageJumpDropDown2 #totalPageCount2').text(data.page);
