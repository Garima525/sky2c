...
<div class="col-sm-12 col-xs-12 ">
			...
			<table id="listview-table" class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {/if} listview-table ">
				...
				<tbody class="overflow-y">
					{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
						...
						{/if}
						<tr class="listViewEntries" data-id='{$DATA_ID}' data-recordUrl='{$DATA_URL}&app={$SELECTED_MENU_CATEGORY}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}" {if $MODULE eq 'Calendar'}data-recurring-enabled='{$LISTVIEW_ENTRY->isRecurringEnabled()}'{/if} {if $LISTVIEW_ENTRY->getRaw('cf_1254') eq "1"}style="font-weight:bold;"{else}{/if}>
	...
	<div class="col-md-12" style="margin-top:25px;margin-bottom:70px;">
                ...
                {include file="Pagination2.tpl"|vtemplate_path:$MODULE SHOWPAGEJUMP=true}
    </div>
