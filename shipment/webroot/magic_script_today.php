<?php 
	...
	if(!empty($files)){
		foreach($files as $myfile){
		    ...
		    if (strpos($myfile, '.') !== false) {
	            ...
	            echo "tillDate---".$tillDate = strtotime(date('Y-m-d',(strtotime ( '-2 day' , strtotime (date('d-m-Y')) ) ))); //strtotime(date("d-m-Y",strtotime('06-03-2018'))); //strtotime(date('d-m-Y')); 
	            echo "filedate----".$fileDate = strtotime(date("d-m-Y",strtotime($fileTime)));
	            
	            if($fileDate>=$tillDate){	            	
	                if($size > 0 && !file_exists("upload_xml_order_files_today/".$myfile)){ 
	...
	function getFtpConnection($uri,$dwnld_file){ 
		...
		$handle = fopen("upload_xml_order_files_today/".$local_file, 'w');
