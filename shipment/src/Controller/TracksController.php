...
class TracksController extends AppController
{
	...
	function trackOrder(){
		...
		$descartes_app_id = isset($this->request->data['descartes_id'])? trim($this->request->data['descartes_id']) :'';
		if(!empty($descartes_app_id)){
			$getUserDetail = $this->Auth->user();
		//	if(empty($getUserDetail) || (!empty($getUserDetail) && $getUserDetail['role']==5)){
				//$orderQuery = $this->OrderPackages->find('all')->where(['OrderPackages.package_number' => $descartes_app_id])->contain(['Orders']);

			//}else{
			$countOrder = strlen($descartes_app_id);
			$pos = strlen($descartes_app_id)-2;
			if($countOrder>9){
				if (strpos($descartes_app_id, '-') !== false) {
					//$descartes_app_id = str_replace('-','',$descartes_app_id);
					$orderID[] = $descartes_app_id;
				}else{
					$descartes_app_id1 = substr($descartes_app_id,0,$pos);
					$descartes_app_id11 = substr($descartes_app_id,$pos);
					$orderID[] = $descartes_app_id1."-".$descartes_app_id11;
				}
			}else{				
				$orderID[] = $descartes_app_id;
				...
				$descartes_app_id2 = substr($descartes_app_id,0,$pos-1);
				$descartes_app_id21 = substr($descartes_app_id,$pos-1);

				$orderID[] = $descartes_app_id1;
				$orderID[] = $descartes_app_id1."-".$descartes_app_id11;

				$orderID[] = $descartes_app_id2;
				$orderID[] = $descartes_app_id2."-".$descartes_app_id21;
			}

			$orderQuery = $this->Orders->find('all')->where(['Orders.descrates_app_id IN ' => $orderID])->contain(['OrderPackages']);
			if($orderQuery->count() <= 0){
				$orderQuery = $this->OrderPackages->find('all')->where(['OrderPackages.package_number IN' => $orderID])->contain(['Orders']);
				if($orderQuery->count() > 0){
					$orderData = $orderQuery->first();
					$orderID = $orderData->order->id;
					$packageID = $orderData->id;
					$actionRedirect = HTTP_ROOT.'tracks/track-package/'.@base64_encode(convert_uuencode($orderID))."/".base64_encode(convert_uuencode($packageID));  
					return $this->redirect($actionRedirect);
				}
			}
	...
	function trackPackage($order_id=null,$package_id=null){		
		...
		if(!empty($orderID) || !empty($pkg_id)){
			...
			$orderPackageDetail = $this->OrderPackages->find('all')->where(['OrderPackages.id' => $pkg_id])->toArray();
		//echo '<pre>';print_r($orderPackageDetail);die;
		...
		$this->set('orderPackageDetail',@$orderPackageDetail);
	}
