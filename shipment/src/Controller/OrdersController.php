...
class OrdersController extends AppController
{
    ...
    public function beforeFilter(Event $event){
        ...
        $this->Auth->allow(['generateQrCode','autoImportOrders','processFileOneToAnother']);
    ...
    public function openOrders(){   
        ...
        if($role=='1' ||  $role=='2'){  
            $orders = $this->Orders->find('all')    
                                   ->where(['Orders.status' => 1])->order(['Orders.created' => 'DESC']);
	...
	/* THIS FUNCTION IS USED TO ACCEPT / REJECT ACTION BY DRIVER ON A SINGLE PRODUCT */
	public function orderAction($orderID,$action){   
		...
		if($role != 4){
			...
			if($this->OrderAssignmentLogs->save($OrderAssignmentLogsData)){
				...
				$assign_by = $orderAssignResults->assign_by;
				$assign_to = $orderAssignResults->assign_to;
				...
				$OrderTrackingData = $this->OrderTracking->newEntity();	
				$OrderTrackingData->order_id = $order_id;
				$OrderTrackingData->order_id = $order_id;
				$OrderTrackingData->package_id = $pkg_id;
				$OrderTrackingData->tracking_description = "$pkg_no has been $action_for_email by $driver_name";
				$OrderTrackingData->assign_by = $assign_by;
				$OrderTrackingData->assign_to = $assign_to;
				$this->OrderTracking->save($OrderTrackingData);
	...
	/* THIS FUNCTION IS USED TO ACCEPT / REJECT ACTION BY DRIVER ON MULTIPLE PRODUCT */
			...
			if(!empty($orderAssignResults)){
				foreach($orderAssignResults as $orderAssignLogVal){
					...
					if($this->OrderAssignmentLogs->save($OrderAssignmentLogsData)){
						...
						$OrderTrackingData->order_id = $order_id;
						$OrderTrackingData->package_id = $pkg_id;
						$OrderTrackingData->type = "assign_log";
						$OrderTrackingData->tracking_description = "$pkg_no has been $action_for_email by $driver_name";
						$OrderTrackingData->assign_by = $assign_by;
						$OrderTrackingData->assign_to = $assign_to;
						$this->OrderTracking->save($OrderTrackingData);
						...
						if($action==4){
							...
							$assignData = $this->OrderAssignmentLogs->find('all')
							  ->where(['OrderAssignmentLogs.order_assignment_id' => $orderAssignLogVal['order_assignment_id']])
							...
							$this->OrderAssignments->updateAll([
									  ...
									  'id' => $orderAssignLogVal['order_assignment_id']
								  ]);  
							$this->OrderAssignmentLogs->updateAll([
									  'status' => 4 
								  ], [
									  'order_assignment_id' => $orderAssignLogVal['order_assignment_id'],
									  'status' => 1
								  ]);   
    ...
    public function AssignedOrders($filter=null){   
        ...
        if($role=='1' ||  $role=='2'){  
            ...
            $orders = $this->Orders->find('all')
                                   ->where(['Orders.status' => 2])->order(['Orders.created' => 'DESC']);
           ...
           $assigned_orders = $this->OrderAssignmentLogs->find('all')
                ...
                ->contain(['OrderAssignments'=>['Orders','OrderPackages']])
                ->group(['OrderAssignmentLogs.order_id'])
               	->order(['OrderAssignmentLogs.created' => 'DESC'])
                ->toArray();
            //echo "<pre>"; print_r($assigned_orders) ; die;
    ...
    public function enroutedOrders($filter=null){   
        ...
        if($role=='1' ||  $role=='2'){  
            ...
            $orders = $this->Orders->find('all')->where(['Orders.status' => 4])
            ->order(['Orders.created' => 'DESC']);
        	$this->set(compact('orders'));
            $this->render('enrouted_orders_admin'); 
          ...
          //  pr($assigned_orders) ; die;
            $this->set(compact('assigned_orders'));     
            $this->render('enrouted_orders_agent'); 
            ...
            $this->set(compact('completed_orders'));
            $this->render('enrouted_orders_driver');
        }
    }
    ...
    public function completedOrders(){   
        ...
        if($role=='1' ||  $role=='2'){  
            ...
            $orders = $this->Orders->find('all')
                                   ...
                                   ->group('Orders.id')->order(['Orders.created' => 'DESC']);
            $this->set(compact('orders'));
            $this->render('open_orders_admin'); 
        }else if($role=='3'){
            $completed_orders = $this->OrderAssignments->find('all')
                ...
                ->contain(['Orders','OrderPackages'])
                ->group('OrderAssignments.order_id')
                ->order(['OrderAssignments.created' => 'DESC'])
                ->toArray();
        ...
        if($role=='1' ||  $role=='2'){ 
		   ...
		   $orderPackages = $this->OrderPackages->find('list',[
			...
			$agentsLists = $this->Users->find('list',[
				...
				->where(['Users.role' =>3,'Users.status' =>1,'Users.parent_id' => $this->Auth->user('id')])
    ...
    public function enroutedOrderDetail($id=null,$user_role=null,$req_user_id=null){
        ...
        if($role=='1' ||  $role=='2'){ 
		   ...
		   $orderPackages = $this->OrderPackages->find('list',[
				...
				->select('OrderPackages.id,OrderPackages.package_title')
				->where(['OrderPackages.status' => 4, 'OrderPackages.order_id' => $id])
				->toArray();   
			//Get All Agent List which is under admin or staff 		
			$agentsLists = $this->Users->find('list',[
					'keyField' => 'id',
			...
			$providers = $this->ShippingCarriers->find('list',[
				...
				->toArray();	
            $this->set(compact('order','orderPackages','agentsLists','providers','driversLists'));
            $this->render('enrouted_order_detail_admin');
       
        }else if($role=='3'){
			
            $order = $this->OrderAssignments->find('all')->contain(['OrderPackages','Orders'=>['OrderDetails','OrderLocations','OrderPackages']])->where(['OrderAssignments.assign_to' => $userID, 'OrderAssignments.order_id' => $id, 'OrderAssignments.status_to' => 4])->toArray();
            ...
            $order = $this->OrderAssignmentLogs->find('all')
                  ->where(['OrderAssignmentLogs.order_id' => $id , 'OrderAssignmentLogs.assign_to' => $userID, 'OrderAssignmentLogs.status' => 5])
                  ->contain(['OrderAssignments' =>['Orders'=>['OrderDetails','OrderLocations','OrderPackages','customer_detail'] , 'OrderPackages']])->toArray();
	...
	function generateQrCode(){
		...
		if ($this->request->is('post')) {
		    $this->loadModel('OrderPackages');
		    $orderID = trim($this->request->data['order_id']);
            $packageNumber = trim($this->request->data['package_number']);
            
            $packageOrderID = $packageID = $orderStatus = "";
            $packages = $orders = array();
            
			$order = $this->Orders->find('all')
			  ->where(['Orders.descrates_app_id' => $orderID])
			  ->toArray();

			if(!empty($order)){
			    foreach($order as $orders){
			        $orderStatus = $orders['status'];
			        $packages = $this->OrderPackages->find('all')
        			  ->where(['OrderPackages.package_number' => $packageNumber,'OrderPackages.order_id'=>$orders['id'] ])
        			  ->toArray();
        			if(!empty($packages)){
			            foreach($packages as $package){
			                $packageID = $package['package_number'];
			            }
        			}
			        $packageOrderID = $orders['descrates_app_id'];
			    }
			}
			
			if(!empty($packageID) && !empty($packageOrderID)){
			    if($orderStatus!="3"){
    			    $QR_Code="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=".$packageID; 
        			$destination = realpath('../webroot/img/QR').'/'.'qr_'.$packageID.'.jpg'; 
        			$this->grab_image($QR_Code,$destination);
        			$finalPath = HTTP_ROOT.'img/QR/'.'qr_'.$packageID.'.jpg';
        			...
        			$destination1 = realpath('../webroot/img/QR').'/'.'bar_'.$packageID.'.jpg'; 
        			$image = $this->barcode($destination1, $text, $size, $orientation, $code_type, $print, $sizefactor );
        			$finalPath = HTTP_ROOT.'img/QR/'.'bar_'.$packageID.'.jpg';
        			
        			$image = '<img src="'.$finalPath.'">';
			    }else{
			        $this->Flash->error(__('Order is already completed.'));
			    }
			}else{
			    $this->Flash->error(__('Invalid Order ID or Package Number.'));
			}
		...
		if(!empty($this->request->data)){
			...
			if($this->request->data['optionsRadios']=='agent'){
				...
				foreach($this->request->data['packages'] as $product_id){
					...
					$pkgData='';				
					$assignData = $this->OrderAssignments->find('all')->where(['OrderAssignments.package_id' => $product_id,'OrderAssignments.assign_to' => $this->request->data['agent']]);
					...
					if($assignData->count() <= 0){
						...
						if($this->OrderAssignments->save($OrderAssignments)){
							...
							$assignToData = $this->getUserById($this->request->data['agent']);
							$assignToName = $assignToData->name;	
							$OrderTrackingData = $this->OrderTracking->newEntity();	
							$OrderTrackingData->order_id = $orderID;
							$OrderTrackingData->type = 'assign_log';
							$OrderTrackingData->package_id = $product_id;
							$OrderTrackingData->tracking_description = "$orderID has been assign to $assignToName";
							$OrderTrackingData->assign_by = $this->Auth->user('id');
							$OrderTrackingData->assign_to = $this->request->data['agent'];		
							$this->OrderTracking->save($OrderTrackingData);
						...
						$UserLogs->notification_type = 'order';
						$UserLogs->description = "New order has been assigned by ".ucwords($this->Auth->user('name'));
						$this->UserLogs->save($UserLogs);
						/*Update Data into log table End*/	
						...
						$dataRec = $assignData->first();
						$OrderAssignments = $this->OrderAssignments->newEntity();
						
						$OrderAssignments->id = $dataRec->id;
						$OrderAssignments->assign_by = $this->Auth->user('id');
						$OrderAssignments->assign_to = $this->request->data['agent'];					
						$OrderAssignments->order_id = $orderID;
						$OrderAssignments->package_id = $product_id;
				...
				foreach($this->request->data['packages'] as $product_id){
					...
					$assignData = $this->OrderAssignments->find('all')->where(['OrderAssignments.package_id' =>$product_id,'OrderAssignments.assign_to' =>$this->Auth->user('id')])->count();
					...
					if($assignData <= 0){
						...
						if($this->OrderAssignments->save($OrderAssignments)){
							...
							$assignToData = $this->getUserById($this->Auth->user('id'));
							$assignToName = $assignToData->name;	
							$OrderTrackingData = $this->OrderTracking->newEntity();	
							$OrderTrackingData->type = 'assign_log';
							$OrderTrackingData->order_id = $orderID;
							...
							$OrderTrackingData->tracking_description = "$orderID has been assign to $assignToName";
							$OrderTrackingData->assign_by =  $this->Auth->user('id');
							$OrderTrackingData->assign_to = $this->Auth->user('id');
							$this->OrderTracking->save($OrderTrackingData);
							...
							if($assignLogData <=0){
								...
								$OrderTrackingData->order_id = $orderID;
								$OrderTrackingData->package_id = $product_id;
								$OrderTrackingData->tracking_description = "$orderID has been assign to 3rdparty";
								$OrderTrackingData->assign_by =  $this->Auth->user('id');
								$OrderTrackingData->assign_to = $this->Auth->user('id');
								$this->OrderTracking->save($OrderTrackingData);
						...
						$assignData = $this->OrderAssignments->find('all')->where(['OrderAssignments.package_id' => $product_id,'OrderAssignments.assign_to' =>  $this->Auth->user('id')]);
				...
				foreach($this->request->data['packages'] as $product_id){
					$pkgData='';				
					$assignData = $this->OrderAssignments->find('all')->where(['OrderAssignments.order_id' => $orderID,'OrderAssignments.package_id' => $product_id,'OrderAssignments.assign_to' => $this->Auth->user('id')])->count();
					/*	
					$assignData = $this->OrderAssignments->find('all')->where(['OrderAssignments.order_id' => $orderID,'OrderAssignments.package_id' => $product_id,'OrderAssignments.assign_to' => $this->Auth->user('id'),'OrderAssignments.status IN' => [1]])->count();
					*/
					if($assignData <= 0){
						$OrderAssignments = $this->OrderAssignments->newEntity();
						...
						if($this->OrderAssignments->save($OrderAssignments)){
							...
							$orderAssignResults= $this->OrderAssignments->find('all')->where(['OrderAssignments.package_id' => $product_id,'OrderAssignments.order_id' => $orderID,'OrderAssignments.assign_to' => $this->Auth->user('id')])->first();
							...
							if($assignData <= 0){
								...
								if($this->OrderAssignmentLogs->save($OrderAssignmentLogs)){
									...
									$assignToData = $this->getUserById($this->request->data['driver']);
									$assignToName = $assignToData->name;	
									$OrderTrackingData = $this->OrderTracking->newEntity();	
									$OrderTrackingData->type = 'assign_log';
									$OrderTrackingData->order_id = $orderID;
									...
									$OrderTrackingData->tracking_description = "$orderID has been assign to $assignToName";
									$OrderTrackingData->assign_by =  $this->Auth->user('id');
									$OrderTrackingData->assign_to = $this->request->data['driver'];
									$this->OrderTracking->save($OrderTrackingData);
						...
						$assignTableData = $this->OrderAssignments->find('all')->where(['OrderAssignments.order_id' => $orderID,'OrderAssignments.package_id' => $product_id,'OrderAssignments.assign_to' => $this->Auth->user('id')])->first();
						if(!empty($assignTableData)){
								...
								$OrderAssignmentLogs->order_id = $orderID;
								$OrderAssignmentLogs->package_id = $product_id;
								$OrderAssignmentLogs->order_assignment_id = $assignTableData->id;
			...
			foreach($this->request->data['packages'] as $product_id){
				...
				if($assignData <= 0){
					...
					if($this->OrderAssignmentLogs->save($OrderAssignmentLogs)){
						...
						$assignToData = $this->getUserById($OrderAssignmentLogs->assign_to);
						$assignToName = $assignToData->name;	
						$OrderTrackingData = $this->OrderTracking->newEntity();	
						$OrderTrackingData->type = 'assign_log';
						$OrderTrackingData->order_id = $orderID;
						$OrderTrackingData->package_id = $product_id;
						$OrderTrackingData->tracking_description = "$orderID has been assign to $assignToName";
						$OrderTrackingData->assign_by =  $OrderAssignmentLogs->assign_by;
						$OrderTrackingData->assign_to = $OrderAssignmentLogs->assign_to;
						$this->OrderTracking->save($OrderTrackingData);
					...
					$OrderAssignmentLogs->id = $assignData->id;
					$OrderAssignmentLogs->assign_by = $this->Auth->user('id');
	...
	function trackOrder($descrates_app_id=null){
		...
		if(!empty($descrates_app_id)){
			...
			$descartesOrderStatas = $this->Orders->find('all')
							  ->where(['Orders.descrates_app_id' => $descrates_app_id])
							  ->contain(['OrderPackages','OrderDescartesTracking'])
							  ->hydrate(false)
							  ->toArray();
		...
		$this->set(compact('trackingRecords','descrates_app_id','msg','descartesOrderStatas'));
	...
	function trackPackage($orderID=null,$pkg_id=null){
		...
		if(!empty($orderID) || !empty($pkg_id)){
			...
			$assignData = $this->OrderAssignmentLogs->find('all')
							   //->where(['OrderAssignmentLogs.order_id' => $orderID,'OrderAssignmentLogs.package_id' => $pkg_id,'OrderAssignmentLogs.status IN' => [2,3,4,5]])
								->where(['OrderAssignmentLogs.order_id' => $orderID,'OrderAssignmentLogs.package_id' => $pkg_id])
				...
				if(strtolower($orderAssignmentData->shipment_type)=='driver'){
					...
					$trackingRecords = $this->Orders->find('all')
									   ...
									   ->contain(['OrderTracking'=> function($q) use($pkg_id) {
																return $q->where(['package_id'=>$pkg_id,'type'=>'tracking_log'])
									   ...
									   ->where(['Orders.id' => $orderID])
									    ->contain(['OrderTracking'=> function($q) use($pkg_id) {
																...
																->contain(['Users'=> function($q) {
																 ...
																 ])->order(['OrderTracking.third_party_status_date' =>'DESC']);
															},
															'OrderPackages'=> function($q) use($pkg_id) {
																return $q->where(['id'=>$pkg_id]);
															}])
					...
					switch ($orderAssignmentData->shipping_carrier->carrier_name) {
							case 'kuebix':
								...
								$this->kuebixTracking($yrcDetails);
								break;
							case 'fedex':
	...
	function importOrders(){
		...
		if(!empty($order_file)){
			...
			//Get xml file data
			$xml = simplexml_load_file(HTTP_ROOT."upload_xml_order_files_new/".$order_file);
			...
			if(empty($file_number)){
				...
				$file_destination = realpath('../webroot/upload_xml_order_files_new').'/'.$order_file;
					...
					if(isset($xml_array['EventCodeSet'])){
						...
						if(!empty($orderDetID) && !empty($packageDetID)){
							...
							$trackingEventCount = $this->OrderTracking->find('all')->where([
								'OrderTracking.order_id'             => $orderDetID,
								'OrderTracking.package_id'           => $packageDetID,
								'OrderTracking.location'             => $locationName.'-'.$locationCode,
								'OrderTracking.country'              => $locationQualifier,
								'OrderTracking.status'               => $eventDescription,
								'OrderTracking.carrier'              => $eventCodeSet.'-'.$eventCode,
								'OrderTracking.tracking_description' => $comment,
								'OrderTracking.third_party_status'   => $eventDescription,
							...
							if($trackingEventCount <= 0){	
								...
								$orderEventData->carrier                  = $eventCodeSet.'-'.$eventCode;
								$orderEventData->country                  = $locationQualifier;
								$orderEventData->location                 = $locationName.'-'.$locationCode;
							...
							foreach($xml_array['Parties']['Party'] as $partyKey=>$partyVal){
								if(count($partyVal)>1){
									...
									if(strtolower($partyType)=='account'){
										...
										$descartes_customer_id = $partyCode;
										$userCount = $this->Users->find('all')->where(['Users.descartes_customer_id' => $descartes_customer_id])->count();
						...
						if(!empty($xml_array['Containers']['Container'])){
							...
							$pkgCount = $this->OrderPackages->find('all')->where(['OrderPackages.package_number' => $package_number,'OrderPackages.order_id' => $new_order_id])->count();
							...
							if($pkgCount<=0){
								...
								$OrderPackagesData['package_title'] =      isset($packageVal['EquipmentTypeCode'])?$packageVal['EquipmentTypeCode']:$file_number;
						...
						if($XML_FILE_STATUS !=''){ // for order tracking
									...
									$orderQuery = $this->Orders->find('all')->where(['Orders.descrates_app_id' => $descartes_order_id])->contain(['OrderPackages'])->first();
									
									if(!empty($orderQuery)){
										
										$Routing = isset($xml_array['Routing'])?$xml_array['Routing']:'';
										//pr($Routing); die;
										if(is_array($Routing) && !empty($Routing)){
											
											foreach($Routing['Segment'] as $sagements){
												
												if(count($sagements)>1){
											
													$ShipmentStatus = isset($xml_array['ShipmentStatus'])?$xml_array['ShipmentStatus']:'';
													$CarriersExecutionDate = isset($xml_array['CarriersExecutionDate'])?$xml_array['CarriersExecutionDate']."00:00:00":'';
												
													
													$orderScannedData = $this->OrderTracking->newEntity();
													$orderScannedData->order_id = $orderQuery->id;
													$orderScannedData->user_id = 0;

													if( strpos( $file_number, "-" ) !== false ) {
														$packageTrackNumber =  $file_number;
													}else if( strpos( $file_number, ":" ) !== false ) {
														$packageTrackNumber =  $file_number;
													}else{
														$packageTrackNumber = $file_number."-01";
													}

													$packageDetTrack = $this->OrderPackages->find('all')->where(['OrderPackages.package_number' => $packageTrackNumber])->first();

													if(!empty($packageDetTrack) && isset($packageDetTrack['id'])){
														$orderScannedData->package_id = $packageDetTrack['id'];
													}else{
														$orderScannedData->package_id = 0;
													}

													/*if(!empty($orderQuery['order_packages'])){
														$orderScannedData->package_id = $orderQuery['order_packages'][0]->id;
													}else{
														$orderScannedData->package_id = 0;
													}*/
													
													
													$orderScannedData->third_party_status = $ShipmentStatus;
													$orderScannedData->status = $ShipmentStatus;
													$orderScannedData->third_party_status_date = isset($sagements['ArrivalDateTime'])?$sagements['ArrivalDateTime']."00:00:00":'';
													$orderScannedData->carrier = isset($sagements['Carrier'])?$sagements['Carrier']:"Descartes";
													
													$orderScannedData->location = isset($sagements['ArrivalLocation'])?$sagements['ArrivalLocation']:'';
													$orderScannedData->third_party_status_date = (isset($sagements['ArrivalDateTime']) && $sagements['ArrivalDateTime'] !='')?date("Y-m-d", strtotime($sagements['ArrivalDateTime'])):'';
													
													$orderScannedData->tracking_description = $sagements['DepartureLocation']." to ".$sagements['ArrivalLocation'];
													
													$trackingCount = $this->OrderTracking->find('all')->where([
														'OrderTracking.order_id' => $orderScannedData->order_id,
														'OrderTracking.package_id' => $orderScannedData->package_id,
														'OrderTracking.location' => $orderScannedData->location,
														'OrderTracking.status' => $orderScannedData->status,
														'OrderTracking.carrier' => $orderScannedData->carrier,
														'OrderTracking.tracking_description' => $orderScannedData->tracking_description,
														'OrderTracking.third_party_status' =>$orderScannedData->third_party_status
													])->count();
													...
													$carrier         = isset($xml_array['Routing']['Segment']['Carrier'])?$xml_array['Routing']['Segment']['Carrier'] : "Descartes";

													$arrivalDateTime = isset($xml_array['Routing']['Segment']['ArrivalDateTime']) ? $xml_array['Routing']['Segment']['ArrivalDateTime'] ."00:00:00" : '' ;

													$arrivalLocation = isset($xml_array['Routing']['Segment']['ArrivalLocation'])? $xml_array['Routing']['Segment']['ArrivalLocation'] : '';

													$arrivalDescription = $xml_array['Routing']['Segment']['DepartureLocation']." to ".$xml_array['Routing']['Segment']['ArrivalLocation'];
													...
													$orderScannedData->third_party_status = $ShipmentStatus;
													$orderScannedData->status = $ShipmentStatus;
													$orderScannedData->third_party_status_date = $arrivalDateTime;
													$orderScannedData->carrier = $carrier;
													
													$orderScannedData->location = $arrivalLocation;
													$orderScannedData->third_party_status_date = (isset($arrivalDateTime) && $arrivalDateTime !='')? date("Y-m-d", strtotime($arrivalDateTime)):'';
													
													$orderScannedData->tracking_description = $arrivalDescription;
	...
	//phpinfo(); die;
		$path = realpath('../webroot/upload_xml_order_files_new');
		...
		if($index > $totalFile){
			...
			if ($order_file === '.' or $order_file === '..') {
				...
				/*
				$source_folder = realpath('../webroot/upload_xml_order_files_new');
	...
	function processFileOneToAnother(){		
		$path = realpath('../webroot/upload_xml_order_files_new/'); //Scan files from upload xml folder
		$order_files = scandir($path); 
		...
		if(!empty($order_files) && count($order_files) > 0){ 
			...
			foreach($order_files as $order_file){
				...
				$file_destination = realpath('../webroot/upload_xml_order_files_new').'/'.$order_file; //File destination with name
    ...
    function kuebixTracking($kuebixDetails,$trackingNumber='KBX-0036062-17'){
				
		$kuebix_status_array = array(
				'Delivered'=>'Delivered',
				'LDG'=>'Loading',
				'ONH'=>'On-hand',
				'ENR'=>'Enroute',
		...
		$trackingNumber = $track_id = $kuebixDetails['tracking_number'];
		
		$curl = curl_init("https://apis.kuebix.com/v1/shipment/track?kuebixBOLs=$trackingNumber");
		...
		$array['trackDetailMap'][$track_id]['currentStatus'];
		
		$shipStatus = strtolower($array['trackDetailMap'][$track_id]['currentStatus']);		
		$carrier = strtolower($array['trackDetailMap'][$track_id]['carrier']);		
		//echo "<pre>"; print_r($array['trackDetailMap'][$track_id]); die;
		//CHECK SUCCESS RESPONSE
		if(strtolower($shipStatus) != strtolower('BOL not found')){
						
			if(!empty($array['trackDetailMap'][$track_id]['historyDetailsList'])){
				
				foreach($array['trackDetailMap'][$track_id]['historyDetailsList'] as $k=>$v){
					$updated_location='';
					$order_tracking_rec = $this->OrderTracking->find('all')
											   ->where([
												'OrderTracking.package_id'=>$kuebixDetails['pkg_id'],
												'OrderTracking.user_id'=>$kuebixDetails['user_id'],
												'OrderTracking.order_id'=>$kuebixDetails['order_id'],
												'OrderTracking.tracking_description'=>$v['description'],
												'OrderTracking.third_party_status'=>$shipStatus])->count();
												
					if($order_tracking_rec <= 0){
						
						$orderScannedData = $this->OrderTracking->newEntity();
						$orderScannedData = $this->OrderTracking->patchEntity($orderScannedData, $this->request->data);
						$orderScannedData->order_id = $kuebixDetails['order_id'];
						$orderScannedData->user_id = $kuebixDetails['user_id'];
						$orderScannedData->package_id = $kuebixDetails['pkg_id'];
						$orderScannedData->third_party_status = $shipStatus;
						$orderScannedData->status = $v['status'];
						$orderScannedData->carrier = $carrier;
						
						$split = explode(" ", $v['description']);
						$updated_location =  $split[count($split)-3]." ".$split[count($split)-2]." ".$split[count($split)-1];
						
						
						$orderScannedData->location = isset($updated_location)?$updated_location:'';
						$orderScannedData->third_party_status_date = ($v['statusDate'] !='')?date("Y-m-d", strtotime($v['statusDate'])):'';
						$orderScannedData->tracking_description = $v['description'];
						
						$this->OrderTracking->save($orderScannedData);
						if($shipStatus=='delivered'){
							
							$this->completedOrderStatus($kuebixDetails['order_id'],$kuebixDetails['pkg_id'],$kuebixDetails['user_id']);
						}
					}	
		...
		}else if(strtolower($shipStatus)==strtolower('BOL not found')){
			...
			$orderScannedData->user_id = $kuebixDetails['user_id'];
			$orderScannedData->package_id = $kuebixDetails['pkg_id'];
			$orderScannedData->third_party_status = 0;
			$orderScannedData->status = 'invalid';
			$orderScannedData->carrier = 'Kuebix';
			$updated_location =  '---';
			$orderScannedData->location = isset($updated_location)?$updated_location:'';
			$orderScannedData->third_party_status_date = ($v['statusDate'] !='')?date("Y-m-d", strtotime($v['statusDate'])):date("Y-m-d");
			$orderScannedData->tracking_description = $array['trackDetailMap'][$track_id]['currentStatus']." (Invalid tracking id)";
			$this->OrderTracking->save($orderScannedData);
		}
	...
	/* list of all assigment of order */
	function assignmentsOrder($orderID=null,$pkg_id=null){
		$assignAgentData = $assignDriverData = array();
		$this->loadModel('OrderAssignmentLogs');
		$this->loadModel('OrderAssignments');
		$this->loadModel('OrderTracking');
		$this->loadModel('Orders');
		...
		if(!empty($orderID) || !empty($pkg_id)){
			...
			$assignAgentData = $this->OrderAssignments->find('all')
							   	->where(['OrderAssignments.order_id' => $orderID,'OrderAssignments.package_id' => $pkg_id,'OrderAssignments.status_by IN'=>array(1,2,4) ,'OrderAssignments.status_to IN'=>array(1,2,4) ])->hydrate(false)->toArray();

			$assignDriverData = $this->OrderAssignmentLogs->find('all')
							   	->where(['OrderAssignmentLogs.order_id' => $orderID,'OrderAssignmentLogs.package_id' => $pkg_id,'OrderAssignmentLogs.status IN'=>array(1,2,5) ])->hydrate(false)->toArray();		
		}	
		$this->set('orderID', $orderID);
		$this->set('pkgID', $pkg_id);
		$this->set('descrates_app_id', @$orderData->descrates_app_id);
		$this->set(compact('assignAgentData','assignDriverData'));
	}
	

	/* function to cancel order */
  	function cancelOrder($orderID=null,$pkg_id=null){
	    $this->loadModel('OrderAssignmentLogs');
	    $this->loadModel('OrderAssignments');
	    $this->loadModel('OrderPackages');
	    $this->loadModel('OrderTracking');
	    ...
	    if($orderID !="" && $pkg_id!=""){ 
	      	...
	      	$assignDriverData = $this->OrderAssignmentLogs->find('all')
	                  ->where(['OrderAssignmentLogs.order_id' => $orderID,'OrderAssignmentLogs.package_id' => $pkg_id,'OrderAssignmentLogs.status IN'=>array(1,2,5) ])->hydrate(false)->toArray();
			if(!empty($assignAgentData)){
				foreach($assignAgentData as $assignAgent){
				  	$notifyTo[] = $assignAgent["assign_to"];
				}
			}
			if(!empty($assignDriverData)){
				foreach($assignDriverData as $assignDriver){
				  	$notifyTo[] = $assignDriver["assign_to"];
				}
			}

	      	if(!empty($notifyTo)){
		      	$orderTracking = $this->OrderTracking->find('all')
	                  ->where(['OrderTracking.order_id' => $orderID,'OrderTracking.package_id' => $pkg_id,'OrderTracking.user_id IN' => $notifyTo,'OrderTracking.assign_by' =>0,'OrderTracking.assign_to' =>0 ])->hydrate(false)->toArray();	
	            ...
	            if(empty($orderTracking)){
					...
					$orderCount = $this->OrderPackages->find('all')
	                  ->where(['OrderPackages.order_id' => $orderID,'OrderPackages.status !='=>1 ])->hydrate(false)->toArray();
			        ...
			        $notifyTo = array_unique(array_filter($notifyTo));
			        for($i=0; $i<count($notifyTo); $i++){
			        	//Start send notification 
						$badgeCount = $this->getNotificationBadgesCount($notifyTo[$i]);
						...
						$params = [
							...
							'order_number' => $orderDet->descrates_app_id,
							'notification_type' => 'order',
							'topic' => 'sky2c'.$notifyTo[$i],
							'notifyId' => $notifyTo[$i]
						];
						$message = $orderDet->descrates_app_id.' Order has been cancelled  ';
			        ...
			        $response = array("status"=>"1","message"=>"Order has been cancelled.");
			    }else{
			    	$response = array("status"=>"0","message"=>"Tracking already started you can't cancel this order.");
			    }
	      	}else{
	      		$response = array("status"=>"0","message"=>"No assignment found for this order.");
	      	}
	    }else{
	      $response = array("status"=>"0","message"=>"Please provide order id and package id both.");
	    }
	    echo json_encode($response);
	    die();
	 }

	function completeOrderStatus($order_id){
		...
		if($role != 3){
			$resultJ = array('status' => '0' , 'message' => "Only agent can perform this action", 'data' => array());	
		}else{
			...
			if(!empty($OrderAssignmentsData)){
				...
				$resultJ = array('status' => '1' , 'message' => "Order has been completed", 'data' => array());
			}else{
				$resultJ = array('status' => '0' , 'message' => "No assignment found.", 'data' => array());
			}
		
		}
		echo json_encode($resultJ);	 die();
 	}
}
?>
