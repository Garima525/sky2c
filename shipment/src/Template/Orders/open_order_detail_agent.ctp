...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		if($this->request->action =='assignedOrderDetail'){
			$orderAction = 'assigned-orders';
		}else if($this->request->action =='openOrderDetail'){
			$orderAction = 'open-orders';
		}
		?>
		<li>
			 <a href="<?= HTTP_ROOT.'orders/'.$orderAction; ?>">Open orders</a>
			<i class="fa fa-circle"></i>
		</li>
		 <li>
      ...
      <!-- BEGIN EXAMPLE TABLE PORTLET-->
        <div class="portlet light bordered">   
            <div class="portlet-title">
				...
				<h4 class="text-primary mt-0"><b>ID - <span><?php if(!empty(@$order[0]->id)){ echo @$order[0]->order->descrates_app_id; }; ?></span></b></h4>
				
				<ul class="list-inline mb-0 pull-right">
					<?php if($this->request->action !='assignedOrderDetail'){  ?>
					<li>
					...
					<li>
						<div class="caption pull-right">
							<a data-href="<?php echo HTTP_ROOT.'orders/complete-order-status/'.@$order[0]->order->id; ?>" href="javascript:void(0)"  data-redirect="<?php echo HTTP_ROOT.'orders/completed-orders';?>"  class="icon-btn btn blue markComplete markCompleteAct "><i class="fa fa-check"></i>
								<div><b> Mark As Complete</b> </div>
							</a>
						</div>
					</li> 
					...
					<li>
						<div class="caption pull-right">
							<a href="<?php echo HTTP_ROOT.'orders/track-order/'.@$order[0]->order->descrates_app_id; ?>" class="icon-btn btn blue"><i class="fa fa-map-marker"></i>
								<div><b> Track</b> </div>
							</a>
						</div>
					</li> 
                      ...
                      <div class="form-body">
                        ...
                        <div class="row">
                          <div class="col-xs-12">
                            ...
                            <thead>
                                <tr>
                                    <th data-orderable="false" class="control sorting_disabled text-center" rowspan="1" colspan="1" aria-label="">
                            ...
                            $order_status = $package['status_to'];
                            ?>
                            <tr>
                                <td class="text-center">
                                   <?php if($order_status == 2 || $order_status == 3){
										...
										<label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
										   <input class="childrens checkboxes" data-assignment-id="<?php echo $package->id; ?>" name="packages" value="<?php echo $package->package_id; ?>" type="checkbox">
										<span></span>
										</label>
										<?php } ?>
                                </td>
                                ...
                                <td class="text-center sorting_1 bold">
                                     <?= $this->Html->link(__($package['order_package']['package_number']), ['action' => 'package-detail', base64_encode(convert_uuencode($package['order_package']['id']))],['title' => 'Packages', 'escape' => false, 'class'=> ''] ) ?>
                                </td>
						...
						<?php if(!empty($order[0]['order']['order_details'])){ ?>
							<div class="ord-widget order_party_detail">
							  ...
							  <div class="order_detail_sec">
								...
								<?php 
								$i=1;
								foreach($order[0]['order']['order_details'] as $order_details_key => $order_details_val){ ?>
                        ...
                        <div class="ord-widget">
                          <div class="row ">
                             <div class="col-sm-12 col-md-6 col-lg-4">
                              ...
                              <div class="well cust-info">
									<h5>Customer info</h5>
									<p><b>Name:</b>  <?=  ($order['0']['order']['customer_detail']['name'] !='')?@ucfirst(@$order['0']['order']['customer_detail']['name']):"---";  ?></p>
									<p><b>Email:</b>  <?=  ($order['0']['order']['customer_detail']['email'] !='')?@ucfirst(@$order['0']['order']['customer_detail']['email']):"---";  ?></p>
									<p><b>Phone:</b>  <?=  ($order['0']['order']['customer_detail']['phone'] !='')?@ucfirst(@$order['0']['order']['customer_detail']['phone']):"---";  ?></p>
							  </div>	
                           ...
                           <div class="col-sm-12 col-md-6 col-lg-4">
                              <div class="well cust-info">
                                ...
                                <p><b>Pick up date:</b> <?= !empty($order['0']['order']['pickup_date']) ? (date('M jS, Y', strtotime( $order['0']['order']['pickup_date'] ) )):"" ?></p>
                                <p><b>Drop off date:</b> <?= !empty($order['0']['order']['drop_off_date']) ? (date('M jS, Y', strtotime( $order['0']['order']['drop_off_date'] ) )):"" ?></p>
                              </div>
                        ...
                        <?php 
                        if(!empty($order[0]['order']['order_locations'])){ ?>
							<div class="ord-widget ">
							  <h5 class="bold text-primary">Order's location details</h5>	
							  
							  <div class="row ">
								
								<?php foreach($order[0]['order']['order_locations'] as $locationkey => $location){ ?>
								<div class="col-sm-12 col-md-6 col-lg-4">
			...
			<div class="modal-body">
				...
				<div class="row">
					<form class="horizontal-form" id="order_assign_to_driver" action="<?php echo HTTP_ROOT; ?>orders/assign-order-packages-by-agent" role="form">
						<input type="hidden" value="<?= @$order[0]['order']['id']; ?>" name="order_id" />
						<input type="hidden" id="current_url" value="<?php echo base64_encode(convert_uuencode(@$order[0]['order']['id'])); ?>" name="current_url" />
						<div class="col-md-12 form-group">
							...
							<select name="packages[]" id="my_packages" multiple="" class="form-control">
								<?php 
								if(!empty($order)){ 
									foreach($order as $orderPkgKey=>$orderPkgVal){
								?>
									<option value="<?php echo $orderPkgVal['order_package']['id']; ?>"><?php echo $orderPkgVal['order_package']['package_title']; ?></option>
								<?php } 
								} ?>
							</select>
						</div>
						...
						<div class="col-md-6 form-group">
							...
							<textarea name="destination" placeholder="" resize class="form-control" rows="2"></textarea>
						</div>						
